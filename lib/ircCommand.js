...
ircCommand.commandParser = function(from, commandLine, callback) {
  ...
  var response;
  if (command === 'join') {
    eventsManager.emit('ircCommand::join', from, sendResponse);
  }
  else if (command === 'tasks') {
    eventsManager.emit('ircCommand::tasks', from, sendResponse);
  }
    ...
    var id = args[0];
    eventsManager.emit('ircCommand::start', from, id, sendResponse);
  }
