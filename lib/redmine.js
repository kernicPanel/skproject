...
redmine.events = function() {
  ...
  eventsManager.on('redmine::ircCommand', function(from, command, args, callback){
    redmine.command(from, command, args, function(err, response) {
...
function ircJoin(from, callback){
...
function ircTasks(from, callback){
...
function ircLongTasks(from, callback){
  User.findOne({id: 4}, 'issue_ids', function (err, user) {
    redmine.getIssues(user.issue_ids, function(err, issuesArray){
      var response = [];
      issuesArray.forEach(function(issue){
        response.push(issue.project.name + ' : ' + issue.subject + ' ' + issue.url);
