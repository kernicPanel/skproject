...
var parseCommand = function(from, message, callback ) {

  commandParser(from, message, function(err, res){
    irc.client.say(from, res);
  });
};
...
var parseMessage = function(from, message, callback ) {
  ...
  else if(currentIrcNicks.indexOf(from) !== -1) {
    parseCommand(from, message);
  }
  else {
    irc.client.say(from, from + ": You're not allowed to run commands");
    irc.client.say(from, "please connect to RealTeam : http://" + server.config.server.host);
    irc.client.say(from, "and setup your irc nickname : http://" + server.config.server.host + '/account');
  }
