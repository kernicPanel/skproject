...
redmine.events = function() {
  ...
  eventsManager.on('redmine::getIrcNicks', function(callback){
    redmine.getIrcNicks(function(err, ircNicks) {
      callback(err, ircNicks);
    });
  });
...
redmine.getIrcNicks = function( callback ) {
  User.where('mail')
  .regex(server.config.redmine.mailFilter)
  .select('irc -_id')
  .exec( function (err, usersArray) {
...
redmine.ircJoin = function(from, callback){
  User.findOne({irc: from}, 'id firstname lastname issue_ids', function (err, user) {
    var response = [];
    response.push('Hello ' + user.firstname + ' ' + user.lastname);
    response.push('http://' + server.config.irc.serverHost + '/#/users/' + user.id);
    response.push(user.issue_ids.length + ' taches assignées');
    console.log('response', response);
    callback(err, response);
  });
};

redmine.ircTasks = function(from, callback){
  User.findOne({irc: from}, 'issue_ids', function (err, user) {
...
/*
 *redmine.command = function( from, command, args, callback ) {
 ...
 *
 *  if (command === 'join') {
 *    ircJoin(from, function(err, response){
 *      callback(null, response);
 *    });
 *  }
 *  else if (command === 'tasks') {
 *    ircTasks(from, function(err, response){
 *      callback(null, response);
 *    });
 *  }
 *  else if (command === 'longtasks') {
 *    ircLongTasks(function(err, response){
 *      callback(null, response);
 *    });
 *  }
 *  else if (command === 'start') {
 *    ircStart(from, args, function(err, response){
 *      callback(err, response);
 *    });
 *  }
 *};
