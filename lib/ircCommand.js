...
ircCommand.commandParser = function(userId, commandLine, callback) {
  ...
  if (command === 'join') {
    ...
    response.push('start <issue id> : start task (unstable)');
    response.push('pause : pause task (coming soon)');
    response.push('stop : stop task (unstable)');
    response.push('addtime <issue id> <time> [comments] : add time intry to task');
