      ...
      Issue.find({ 'assigned_to.name': doc.firstname + ' ' + doc.lastname }, ['id'])
        ...
        .run(function (err, docs) {
          ...
          for (var i=0; i < docs.length; i++) {
            var doc = docs[i];
            teamMember.issue_ids.push(doc.id);
          }
          //teamMember.issues = docs;
          teamMember.issuesCount = docs.length;
          teamMember.redmine.issues = docs;
          teamMember.redmine.issuesCount = docs.length;
          teamMember.save(function () {
          eventsManager.emit('log', teamMember);
        });
      });
    }
  });
...
redmine.getTeamMember = function( id, callback ) {
  TeamMember.findOne({id: id}, 'id name issuesCount issue_ids', function (err, teamMember) {
    callback(err, teamMember);
  });
};

redmine.getTeamMembers = function( callback ) {
  TeamMember.find({}, 'id name issuesCount issue_ids current', function (err, teamMembersArray) {
    callback(err, teamMembersArray);
  });
};
...
redmine.getIssue = function( id, callback ) {
  Issue.findOne({ id: id  }, function (err, doc) {
    callback(null, doc);
  });
};
...
redmine.updateCurrentIssue = function updateCurrentIssue ( currentIssue, callback ) {
  TeamMember.findOne({login: currentIssue.login}, function (err, user) {
    //callback(null, true);
