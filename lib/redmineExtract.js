...
console.log("»»»»»redmineExtract load««««««".verbose);
var redmineExtract ={};
...
redmineExtract.events = function() {
    ...
    eventsManager.on('redmineExtract::getGarantie', function(callback){
        redmineExtract.getGarantie(function(err, data) {
            callback(err, data);
        });
    });

    eventsManager.on('redmineExtract::getSupport', function(callback){
        redmineExtract.getSupport(function(err, data) {
            callback(err, data);
        });
    });
...
redmineExtract.getIssues = function(callback) {
    ...
    stream.on('close', function () {
        ...
        eventsManager.emit('redmineExtract::getIssues::done');
        callback();
    });
};

redmineExtract.getSupport = function(callback) {
    var team = config.extract.team;
    console.log("redmineExtract getIssues : ");
    var query = IssueExtract.find({});
    query.sort('project.name', 1);
...
redmineExtract.getGarantie = function(callback) {
    var team = config.extract.team;
    console.log("redmineExtract getIssues : ");
    var query = IssueExtract.find({});
    query.sort('project.name', 1);
