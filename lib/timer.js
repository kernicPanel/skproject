...
timer.events = function() {
  ...
  eventsManager.on('timer::ircPause', function(userId, callback){
    timer.ircPause(userId, function(err, data) {
      callback(err, data);
    });
  });
...
timer.ircPause = function(userId, callback) {
  timer.getUserFromId( userId, function( err, user ){
    user.pauseIssue( function( err, issue ) {
      ...
      if (issue.id) {
        ...
        eventsManager.emit('timer::issuePaused', currentIssue);
        callback(null, currentIssue);
      }
      else {
...
timer.ircAddtime = function ircAddtime (userId, issue, callback) {
  ...
  timer.getUserFromId( userId, function( err, user ){
    ...
    user.redmineRest.request('POST', '/time_entries.json', params, function(err, data) {
      console.log("ircAddtime data error : ", err ? err.message:err);
      data = data || params;
      data.time_entry.fullHours = params.time_entry.hours;
      console.log("ircAddtime data response : ", data);
      callback( err ? err.message: err, data);
    });
    //callback();
  });
};
...
timer.addtime = function addtime (userId, issue, callback) {
  ...
  timer.getUserFromId( userId, function( err, user ){
    ...
    user.redmineRest.request('POST', '/time_entries.json', params, function(err, data) {
      ...
      if (err) {
        console.log(err);
        eventsManager.emit('timer::addTimeError', err.message, params);
      }
      else {
        eventsManager.emit('timer::addTimeOk', data);
      }
      callback( err ? err.message:err, data );
    });
    callback();
