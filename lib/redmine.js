...
redmine.init = function() {
    ...
    //io.sockets.on('connection', function(socket){
        redmine.getUsersIssues( function(err, data) {
            server.emit('getUsersIssues::response', data);
        });

        server.emit('redmine::connect');

        server.on('redmine::sync', function(data){
            redmine.sync( function(err, data) {
                //server.emit('redmine::response', data);
            });
            //server.broadcast.emit('server_message',data);
            //server.emit('server_message',data);
        });

        server.on('getDatabaseState', function(){
            server.emit('databaseState', {state: mongoose.connection.readyState});
        });

        server.on('setUsersIssues', function(){
            redmine.setUsersIssues( function(err, data) {
                //server.emit('redmine::response', data);
                server.emit('log', data);
            });
        });
        server.on('getUsersIssues', function(callback){
            redmine.getUsersIssues( function(err, data) {
                server.emit('getUsersIssues::response', data);
        ...
        server.on('setSkProjects', function(){
        ...
        server.on('getSkProjects', function(){
            redmine.getSkProjects( function(err, data) {
                server.emit('getSkProjects::response', data);
                //server.emit('log', data);
            });
        });
    //});
