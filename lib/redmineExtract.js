...
redmineExtract.buildStats = function(callback) {
    ...
    stream.on('data', function (doc) {
        ...
        /*
         *doc.getFirstJournal(function(err, data) {
         *    console.log("err : ", err);
         *    console.log("data : ", data);
         *});
         */
        doc.stats.dateFirstPost = null;
        doc.stats.delaiFirstPost = null;
        doc.stats.userFirstPost = null;
        doc.stats.dateAValider = null;
        doc.stats.delaiAValider = null;
        doc.stats.dateALivrer = null;
        doc.stats.delaiALivrer = null;
        doc.stats.dateLivre = null;
        doc.stats.delaiLivre = null;
        doc.stats.dateFerme = null;
        doc.stats.delaiFerme = null;
        doc.save();
        ...
        Journal.firstPost(journalsID, function(err, journal) {
            if (journal) {
                ...
                doc.stats.dateFirstPost = journal.created_on;
                doc.stats.delaiFirstPost = journal.created_on - doc.created_on;
                doc.stats.userFirstPost = journal.user.name;
                doc.save();
            }
        });
        Journal.firstAValider(journalsID, function(err, journal) {
            if (journal) {
                ...
                doc.stats.dateAValider = journal.created_on;
                doc.stats.delaiAValider = journal.created_on - doc.created_on;
                doc.save();
            }
        });
        Journal.firstALivrer(journalsID, function(err, journal) {
            if (journal) {
                ...
                doc.stats.dateALivrer = journal.created_on;
                doc.stats.delaiALivrer = journal.created_on - doc.created_on;
                doc.save();
            }
        });
        Journal.firstLivre(journalsID, function(err, journal) {
            if (journal) {
                ...
                doc.stats.dateLivre = journal.created_on;
                doc.stats.delaiLivre = journal.created_on - doc.created_on;
                doc.save();
            }
        });
        Journal.firstFerme(journalsID, function(err, journal) {
            if (journal) {
                ...
                doc.stats.dateFerme = journal.created_on;
                doc.stats.delaiFerme = journal.created_on - doc.created_on;
                doc.save();
            }
        });
    });
