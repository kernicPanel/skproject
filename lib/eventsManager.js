...
eventsManager.connect = function(socket) {
  ...
  server.addUser( username, function(){} );
    ...
    socket.on('redmineStats::buildStats', function(callback){
        console.log("eventsManager redmineStats::buildStats : ");
        eventsManager.emit('redmineStats::buildStats', function(err, data){
            callback(err, data);
        });
    });
