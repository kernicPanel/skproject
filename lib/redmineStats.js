...
redmineStats.getIssuesStats = function(settings, callback) {
    ...
    Project.find(projectFilter, 'id', function (err, data) {
        ...
        var query = Issue.find({});
        query.select('id url estimated_hours time_entries_total done_ratio priority assigned_to status');
        query.sort('project.name');
        // query.sort('created_on');
        query.sort('assigned_to.name');
        if (projectId !== 'All') {
            projectsIds.push(projectId);
            query.where('project.id').in(projectsIds);
        }
...
redmineStats.storeAllIssues = function( callback ) {
    ...
    var storeAllItems = function() {
        ...
        redmineRest.getItems('issues', {status_id: '*', include:'journals', limit: 100, offset: lastItem}, function(err, data) {
            if (err) {
                console.error("storeAllIssues error : ", err);
                return;
            }
            itemsCount = data.total_count;
            ...
            for (var i = 0; i < loopCount; i++) {
                ...
                if (!!items[i].description) {
                    items[i].description = items[i].description.replace(/</g, '&lt;');
                    items[i].description = items[i].description.replace(/>/g, '&gt;');
                ...
                if (items[i].id === 7922) {
                    eventsManager.emit('log', lastItem);
                    eventsManager.emit('log', items[i]);
                }

                var item = new Issue( items[i] );
                if (!!items[i].assigned_to && !!items[i].assigned_to.id) {
                  item.user_id= items[i].assigned_to.id;
                }
                // item.redmine_id = items[i]['id'];
                // item.type_id = 'issues' + '_' + items[i]['id'];
                //console.log("item : ", item);
                ...
                //console.log("item.priority.id : ", item.priority.id);
                //var priority = new Priority ( item.priority );
                //priority.save();
                /*
                 *Priority.findOne({id: item.priority.id}, function(err, priority){
                 *  console.log("err : ", err);
...
redmineStats.storeAllNotes = function( callback ) {
    ...
    stream.on('data', function (issue) {
      stream.pause();
      redmineRest.getIssueParamsAddData(issue.id, {include: 'journals'}, function(err, data) {

        if (err) {
            console.error("storeAllNotes error : ", issue.id, err);
        }

        if (!!data && data.issue.journals.length) {
          ...
          for (var j = 0; j < jLoopCount; j++) {
            ...
            journal.issueId = issue.id;
            console.log("storeAllNotes journal.id : ", j + 1, '/', jLoopCount, ' ', journal.id);
            journal.save();
          ...
          issue.save(function(err, issue){
            console.log("issue : ", counter, issue.id, issue.subject, 'SAVED');
            stream.resume();
          });
          ...
          if(!data){
            console.error("no data : ", issue.id);
          }
          eventsManager.emit('syncPending', {
            type: 'issue journals',
            text: Math.round(++counter / total * 100),
            message: issue.id + ' ' + issue.subject + ' no journal'
          });

          //console.log("issue : ", counter, issue.id, issue.subject, 'no journal');
...
redmineStats.storeAllTimeEntries = function( callback ) {
    ...
    stream.on('data', function (issue) {
      stream.pause();
      redmineRest.getTimeEntriesParamsAddData({issue_id: issue.id.toString()}, function(err, data) {

        if (err) {
            console.error("storeAllTimeEntries error : ", issue.id, err);
        }

        console.log("data : ", data);
        if (!!data && data.time_entries.length) {
          eventsManager.emit('syncPending', {
            type: 'issue timeEntries',
            text: Math.round(++counter / total * 100)
          });

          issue.time_entries_total = 0;
          ...
          eventsManager.emit('syncPending', {
            type: 'issue timeEntries',
            text: Math.round(++counter / total * 100),
            message: issue.id + ' ' + issue.subject + ' no timeEntry'
          });
