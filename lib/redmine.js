...
redmine.getUser = function( id, callback ) {
  TeamMember.findOne({id: id}, 'id name issuesCount issues', function (err, user) {
    callback(err, user);
  });
};

redmine.getUsers = function( callback ) {
  TeamMember.find({}, 'id name issuesCount issues', function (err, usersArray) {
