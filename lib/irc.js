...
var joinMessage = function(from, message, pm ) {
  var recipient = pm ? from : channel;
    ircCommand.commandParser(currentIrcNicks[from], 'invite', function(err, response){
      response.forEach(function(line){
        irc.client.say(recipient, line);
      });
    });
...
irc.init = function() {
  ...
  irc.client.addListener('join' + channel, function (from, message){
    ...
    if (from != server.config.irc.botName) {
...
irc.pMessage = function(user, message) {
  irc.client.say(user, message);
}
