...
redmine.events = function() {
  ...
  eventsManager.on('redmine::getIrcUsers', function(callback){
    redmine.getIrcUsers(function(err, ircUsers) {
      callback(err, ircUsers);
    });
  });

  /*
   *eventsManager.on('redmine::ircJoin', function(from, callback){
   *  redmine.ircJoin(from, function(err, response) {
   *    callback(err, response);
   *  });
   *});
   */

  eventsManager.on('redmine::ircTasks', function(from, callback){
    redmine.ircTasks(from, function(err, response) {
      callback(err, response);
    });
  });

  eventsManager.on('redmine::ircTask', function(from, callback){
    redmine.ircTask(from, function(err, response) {
      callback(err, response);
    });
  });
...
redmine.fetchRedmineData = function( ItemModel, type, callback) {
  ...
  var getAllItems = function() {
    ...
    redmineRest.getItems(type, {limit: 100, offset: lastItem}, function(err, data) {
      if (err) {
        console.log("redmine.fetchRedmineData error : ", err.message);
        return;
      }
...
redmine.truc = function( callback ) {
  callback(true);
};
...
redmine.getUsers = function( callback ) {
  ...
  User.where('mail')
  .regex(server.config.redmine.mailFilter)
  .select('id mail firstname lastname issue_ids current_issue_id current_issue_time current_issue_status current_issue_started_at -_id')
...
redmine.getIrcUsers = function( callback ) {
  User.where('mail')
  .regex(server.config.redmine.mailFilter)
  .select('id irc -_id')
  .exists('irc')
...
/*
 *redmine.ircJoin = function(userId, callback){
 *};
 */

redmine.ircTasks = function(userId, callback){
  User.findOne({id: userId}, 'issue_ids', function (err, user) {
    redmine.getIssues(user.issue_ids, function(err, issuesArray){
      var response = [];
      issuesArray.forEach(function(issue){
        response.push(issue.project.name + ' : ' + issue.subject + ' ' + issue.url);
      });
...
redmine.ircTask = function(userId, callback){
  //User.findOne({id: userId}, 'issue_ids', function (err, user) {
  User.findOne({id: userId}, 'currentIssue current_issue_id current_issue_started_at current_issue_status current_issue_time', function (err, user) {
    console.log('user', user);
    if (!!user.currentIssue.id) {
      redmine.getIssue(user.currentIssue.id, function(err, issue){
        var response = [];
        var now = new Date;
        //var duration = moment( Math.round(time_entry.fullHours*60*60*1000)).zone(0).format('H:mm:ss');
        if (user.currentIssue.paused) {
          var diff = parseInt(user.currentIssue.pendingDuration, 10);
        }
        else {
          var diff = now - parseInt(user.currentIssue.startedAt,10) + parseInt(user.currentIssue.pendingDuration, 10);
        }
        console.log('diff', diff);
        var duration = moment( diff).zone(0).format('H:mm:ss');
        //var duration = moment().from(user.current_issue_started_at).format('H:mm:ss');
        response.push(issue.project.name + ' : ' + issue.subject + ' ' + issue.url);
        response.push(duration + ' : ' + user.currentIssue.issueStatus);
        console.log('response', response);
        callback(err, response);
      });
    }
        ...
        var response = [];
        response.push('No issue started');
        callback(err, response);
    }
  });
};
...
redmine.getStatsIssuesByType = function( ids, type, callback ) {
  var stats = [];
...
redmine.getAllStatsIssues= function( ids, callbackStats ) {
  var stats = {};
  ...
  flow.series([
    function (callback) {
      stats.status = [];
      o.map = function () {
        emit(this.status.name, 1);
      };
      Issue.mapReduce(o, function (err, results) {
        for (var i=0; i < results.length; i++) {
          var result = results[i];
          stats.status.push([result._id, result.value]);
        }
        //console.log(err);
        //console.log(results);
        callback();
    ...
    function (callback) {
      stats.priority = [];
      o.map = function () {
        emit(this.priority.name, 1);
      };
      Issue.mapReduce(o, function (err, results) {
        for (var i=0; i < results.length; i++) {
          var result = results[i];
          stats.priority.push([result._id, result.value]);
        }
        callback();
      });
    },
    function (callback) {
      stats.project = [];
      o.map = function () {
        emit(this.project.name, 1);
      };
      Issue.mapReduce(o, function (err, results) {
        for (var i=0; i < results.length; i++) {
          var result = results[i];
          stats.project.push([result._id, result.value]);
        }
        callback();
      });
    },
    function (callback) {
      callbackStats(null, stats);
    }
  ]);
  ...
  if (redmineIssue.hasOwnProperty('journals')) {
    ...
    Issue.findOne({id: redmineIssue.id}, function(err, issue){
      ...
      issue.save(function(){
        ...
        eventsManager.emit('updateIssue', issue.id);
  ...
  User.findOne({ id: userInfos.id }, {}, {lean: false}, function (err, user) {
    if (!user) {
      ...
      //user.update(userInfos);
        ...
        user.irc = userInfos.irc;
    }
    user.save(function(err){
      if (user.irc !== null) {
        eventsManager.emit('redmine::ircNick', user.id, user.irc);
      }
      callback(err, user);
    });
  });
...
redmine.updateUser = function createUser ( login, userInfos, callback ) {
  console.log('login', login);
  console.log('userInfos', userInfos);
  User.findOne({ id: userInfos.id }, {}, {lean: false}, function (err, user) {
    if (user.login === login) {
      user.login = userInfos.login;
      user.firstname = userInfos.firstname;
      user.lastname = userInfos.lastname;
      user.mail = userInfos.mail;
      user.irc = userInfos.irc;
      user.save(function(err){
        if (user.irc !== null) {
          eventsManager.emit('redmine::ircNick', user.irc);
        }
        callback(err, user);
      });
    }
...
redmine.connectUser = function connectUser ( login, callback ) {
  ...
  User.findOne( { login: login }, function (err, user) {
    if (user) {
      ...
      //TODO redirect login
      console.log('need login redirect'.error);
      callback(err, false);
    }
  });
};
...
server.post("/redmine/push", function (req, res) {
  ...
  if (event.type==='issue') {
    ...
    event.changes.forEach(function(change){
      ...
      if (change.property === "assigned_to_id") {
        User.findOne({id: change.value}, 'irc', function (err, user) {
          if (!!user) {
            var ircNick = user.irc;
            //console.log('err', err);
            //console.log('ircNick', ircNick);
            if (!!ircNick) {
              redmine.getIssue(event.issue, function(err, issue){
                console.log('issue', issue);
                ...
                server.irc.pMessage(ircNick, message);
              });
            }
          }
        });
