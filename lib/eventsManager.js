...
eventsManager.connect = function(socket) {
  ...
  server.addUser( username, function(){} );
    ...
    eventsManager.once('timer::getCurrentIssues::response', function(issue, callback){
    ...
    eventsManager.on('irc::updateCurrentIssue', function(currentIssue){
        ...
        socket.emit('irc::currentIssueUpdated', currentIssue);
    });
    ...
    // eventsManager.on('timer::startCurrentIssue', function(username){
    //     socket.emit('startCurrentIssue', username);
    // });
    ...
    // eventsManager.on('timer::pauseCurrentIssue', function(username){
    //     socket.emit('pauseCurrentIssue', username);
    // });
    ...
    eventsManager.on('timer::stopCurrentIssue', function(username){
        socket.emit('stopCurrentIssue', username);
    });
