...
redmine.getUserFromKey = function getUserFromKey ( key, callback ) {
  console.log("key : ", key);
  //return redmineRest.getUserFromKey( key, callback );
...
redmine.getAppUser = function getAppUser ( request, callback ) {
  AppUser.findOne( request, function (err, doc) {
    callback(err, doc);
  });
};

redmine.connectUser = function connectUser ( username ) {
  AppUser.findOne( { login: username }, function (err, appUser) {
...
redmine.disconnectUser = function disconnectUser ( username ) {
