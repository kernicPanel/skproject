...
var Users = new Schema({
    ...
    current_issue_time: String,
    current_issue_status: String,
    current_issue_started_at: Date,
    currentIssue: {
      id: { type: String, default: null },
      startedAt: { type: String, default: 0 },
...
Users.methods.startIssue = function( issue, callback ){
  ...
  this.currentIssue.startedAt = new Date().getTime();
  this.current_issue_id = this.currentIssue.id;
  this.current_issue_status = this.currentIssue.issueStatus;
  this.current_issue_time = this.currentIssue.issueTime;
  this.current_issue_started_at = this.currentIssue.startedAt;
  this.save();
...
Users.methods.pauseIssue = function( callback ){
  if (this.currentIssue.paused) {
    ...
    this.currentIssue.pendingDuration = this.currentIssue.pendingDuration + (new Date().getTime() - this.currentIssue.startedAt);
...
var Issues = new Schema({
    ...
    time_entries_total: { type: Number, default: 0 },
    spent_hours: Number,
...
Issues.pre('save', function(next) {
    this.url = 'https://' + config.redmine.host + '/issues/' + this.id;
    ...
    Priority.findOne({id: issue.priority.id}, function(err, priority){
      ...
      if (!priority) {
        ...
        priority = new Priority ( {id: issue.priority.id, name: issue.priority.name} );
        priority.save();
      }
    });
    Status.findOne({id: issue.status.id}, function(err, status){
      //console.log("err : ", err);
      //console.log("status : ", status);
      if (!status) {
        //console.log("status : ", status);
        //console.log("issue.status : ", issue.status);
        status = new Status ( {id: issue.status.id, name: issue.status.name} );
        status.save();
      }
    });
    next();
