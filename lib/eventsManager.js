...
eventsManager.on('irc::getIrcNicks', function(callback){
    eventsManager.emit('redmine::getIrcNicks', function(err, ircNicks){
        callback(err, ircNicks);
    });
});

eventsManager.on('redmine::ircNick', function(ircNick){
    eventsManager.emit('irc::addIrcNick', ircNick);
});
...
eventsManager.on('ircCommand::join', function(from, callback){
  eventsManager.emit('redmine::ircJoin', from, function(err, response){
    callback(err, response);
  });
});

eventsManager.on('ircCommand::tasks', function(from, callback){
  eventsManager.emit('redmine::ircTasks', from, function(err, response){
    callback(err, response);
  });
});
...
eventsManager.on('ircCommand::start', function(from, args, callback){
  eventsManager.emit('timer::ircStart', from, function(err, response){
    callback(err, response);
  });
});
...
eventsManager.connect = function(socket) {
  ...
  server.addUser( login, function(){} );
    ...
    eventsManager.on('timer::stopCurrentIssue', function(userId){
        socket.emit('stopCurrentIssue', userId);
    });
    ...
    eventsManager.on('updateIssue', function(issueID, detail){
        ...
        socket.emit('updateIssue', issueID, detail);
    });

    eventsManager.on('addUserIssue', function(data){
        //console.log("eventsManager updateIssue : ");
        socket.emit('addUserIssue', data);
    });

    eventsManager.on('removeUserIssue', function(data){
        //console.log("eventsManager updateIssue : ");
        socket.emit('removeUserIssue', data);
    });
