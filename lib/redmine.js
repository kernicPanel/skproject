...
redmine.events = function() {
  ...
  eventsManager.on('redmine::setUsersIssues', function(callback){
  ...
  eventsManager.on('redmine::setSkProjects', function(){
    console.log("setSkProjects : ");
    redmine.setSkProjects( function(err, data) {
      eventsManager.emit('log', data);
    });
  });
  eventsManager.on('redmine::getSkProjects', function(){
...
redmine.getAllRedmineUsers = function( callback ) {
...
redmine.getUsers = function( callback ) {
  TeamMember.count({}, function (err, total) {
    var query = TeamMember.find({});
    ...
    stream.on('data', function (user) {
      ...
      console.log("user watta watta : ", user.id);
      eventsManager.emit('getUsers::data', {user:user, progress:progress});
      // callback(null, {user:user, progress:progress});
    });
...
redmine.getAllUsers = function( callback ) {
  TeamMember.find({}, function (err, usersArray) {
    //eventsManager.emit('getUsers::response', usersArray);
    var users = {};
...
redmine.getIssues = function( callback ) {
  Issue.find({}, function (err, issuesArray) {
    // doc.description = textile(doc.description);
    // eventsManager.emit('log', issuesArray);
...
redmine.syncIssues = function() {
  redmine.checkLastIssue();
  setInterval(redmine.checkLastIssue, 10 * 1000);
};

redmine.checkLastIssue = function(offset) {
  // console.log(" ========== »»»» redmine.checkLastIssue : ");
  offset = offset || 0;
    ...
    if (data && data.issues) {
      ...
      Issue.findOne({ 'id': redmineIssue.id }, function (err, doc) {
        ...
        if (doc) {
          ...
          redmine.createIssue(redmineIssue);
          redmine.checkLastIssue(++offset);
        }
      });
    }
...
redmine.createIssue = function(redmineIssue) {
...
redmine.updateIssue = function(redmineIssue, callback) {
  console.log("updating issue : ", redmineIssue.id, redmineIssue.assigned_to.name);
...
redmine.testIssueUpdate = function testIssueUpdate (currentAssignation, callback) {
  ...
  // eventsManager.emit('log', issue.assigned_to);
